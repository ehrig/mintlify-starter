...
The **Query Loop** builder is available for all [layout elements](https://academy.bricksbuilder.io/article/layout/), Accordion, and Slider elements.
...
![](images/container-query-loop-control.png)
...
![](images/query-control-object-type.png)

- **Posts** enable a [WP\_Query](https://developer.wordpress.org/reference/classes/wp_query/) type of query. This is the default query type and should be used when you want to display a loop of posts, pages, media files, or custom post types.

- **Terms** enable the [WP\_Term\_Query](https://developer.wordpress.org/reference/classes/wp_term_query/). This should be used when you want to loop through the different terms of a taxonomy. Useful to list all the product categories that contain products.

- **Users** enable the [WP\_User\_Query](https://developer.wordpress.org/reference/classes/WP_User_Query/). This should be used when you want to loop through a set of site users. Useful to list the blog authors or a list of team members (as long as they are inserted as site users).
...
Note: You must enable code execution in [Bricks settings](https://academy.bricksbuilder.io/article/settings/) to access this feature.



![](images/bricks-1.9.1-query-editor-php.png)
...
**Disable Query Merge**: Turn this on if do not want the query to be auto-merged by Bricks in archive pages, search pages, etc. Usually, you will turn this on for the Query loops in the footer, header, or non-main query. This is the GUI for the [bricks/posts/merge\_query](https://academy.bricksbuilder.io/article/filter-bricks-posts-merge_query/) filter.

**Child Of**: Set the parent ID to return all its children only. (`post_parent` in WP\_Query)
...
![](images/enhanced-query-loop-order-by-example-1.png)
...
- **Custom Order Clauses with Meta Queries**: Bricks now supports more complex ordering directly aligned with meta query conditions. For instance, if you’re working with a meta query to order posts by **performance date** and **time**, you can define this directly in the order clause. Code example in [this article](/article/filter-bricks-posts-query_vars/#orderby-with-multiple-fields).

![](images/enhanced-query-loop-order-by-example-2.png)
...
![Custom Query Loop Builder - latest posts](images/container-loop-latest-posts-1024x399.png)
...
After selecting `Media` in the post type control, you'll get a new control to define the mime type. By default, Bricks automatically queries for images, but you may define other mime types (separated by a comma, e.g., _image/jpeg,image/png,image/gif_).
...
![](images/bricks-media-query-loop-settings.png)
...
![](images/bricks-media-query-loop-1024x618.jpg)
...
**Current post term**: Enable to get the terms assigned to the current post only. `(@since 1.8.4)` Only use in single post context. Only visible if "Type" is set to "Term". This is the same logic as the example in [bricks/terms/query\_vars](/article/filter-bricks-terms-query_vars/#get-terms-assigned-to-a-post)

![](images/term-query-current-post-term.png)
...
![](images/bricks-query-loop-terms-1024x499.png)
...
![](images/bricks-query-terms-bg-image.png)
...
![](images/bricks-query-terms-link-1.png)

## Users query

![](images/bricks-query-control-users.png)
...
![](images/bricks-query-loop-users-1024x467.png)
...
![](images/bricks-pagination-element.png)
...
![](images/bricks-pagination-related-query.png)

Tip: to make it easier to recognize elements, give descriptive element names to the containers that have a query enabled.

## Load more (button)

Besides the infinite scroll, which automatically loads more results as you scroll down the page, you can also give any element (typically the Button) a "Load more" functionality by adding a "Load more" click [interaction](https://academy.bricksbuilder.io/article/interactions/) to it like this:

![](images/bricks-query-loop-load-more-button-scaled.jpg)
...
![](images/bricks-query-loop-accordion.png)
...
![](images/bricks-query-control-slider.png)
...
![](images/query-loop-include-supports-dynamic-values.png)
...
![](images/query-loop-include-supports-dynamic-values-example-1.png)
...
![](images/query-loop-include-supports-dynamic-values-example-2.png)
...
![](images/query-loop-include-supports-dynamic-values-example-3.png)

![](images/query-loop-include-supports-dynamic-values-example-3-result.png)

## Query loop hooks

- [bricks/query/run](/article/filter-bricks-query-run/) (filter)

- [bricks/terms/query\_vars](/article/filter-bricks-terms-query_vars/) (filter)

- [bricks/users/query\_vars](/article/filter-bricks-users-query_vars/) (filter)

- [bricks/posts/merge\_query](/article/filter-bricks-posts-merge_query/) (filter)

- [bricks/posts/query\_vars](/article/filter-bricks-posts-query_vars/) (filter)

- [bricks/query/loop\_object](/article/filter-bricks-query-loop_object/) (filter)

- [bricks/query/loop\_object\_id](/article/filter-bricks-query-loop_object_id/) (filter)

- [bricks/query/loop\_object\_type](/article/filter-bricks-query-loop_object_type/) (filter)

- [bricks/query/no\_results\_content](/article/filter-bricks-query-no_results_content/) (filter)

- [bricks/query/before\_loop](https://academy.bricksbuilder.io/article/action-bricks-query-before_loop/) (action)

- [bricks/query/after\_loop](/article/action-bricks-query-after_loop/) (action)

- [bricks/query/result](/article/filter-bricks-query-result/) (filter)

- [bricks/query/result\_count](/article/filter-bricks-query-result_count/) (filter)

- [bricks/query/result\_max\_num\_pages](/article/filter-bricks-query-result_max_num_pages/) (filter)

- [bricks/query/init\_loop\_index](/article/filter-bricks-query-init_loop_index/) (filter)
