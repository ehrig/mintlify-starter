...
- `$image` _(string)_ – The default placeholder image path or URL.

- `$is_svg` _(bool)_ – Whether the placeholder should be an SVG. Leave as is.

- `$format` _(string)_ – Can be `'url'` or `'path'` depending on the context Bricks needs it for. Leave as is.
