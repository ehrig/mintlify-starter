...
This is an alternative to setting an active template via the [bricks/screen\_conditions/scores](https://academy.bricksbuilder.io/article/filter-bricks-screen_conditions-scores/) filter.
...
Since Bricks 1.12, templates applied to a page are now also displayed in the Builder. Previously, only the _Post Content_ element was visible, making it difficult to style surrounding elements. If you prefer the old behavior and want to disable the applied template _only inside the builder_, you can use this filter:
